...
void GetMichiRotXY(sceVu0FVECTOR vec, sceVu0FVECTOR rot) {
    ...
    rot[0] = -_atan2(GetVectorLenXZ(vec), sa) + KL2_PI2;
    if (rot[0] < -KL2_PI) {
        rot[0] += KL2_2PI;
    } else if (rot[0] > KL2_PI) {
        rot[0] -= KL2_2PI;
    }

    rot[1] = _atan2(vec[0], vec[2]) + KL2_PI;
    if (rot[1] < -KL2_PI) {
        rot[1] += KL2_2PI;
    } else if (rot[1] > KL2_PI) {
        rot[1] -= KL2_2PI;
    }
}
...
f32 GetMichiSaka(RT_WRK *rtw, s32 michimuki) {
    ...
    vf[2] = xzlen;
    vf[0] = KL2_VER_COND(atan2f, _atan2)(vf[1], vf[2]);
    return vf[0];
}
...
s32 MichiBunki2Cir(RT_WRK *rtw, sceVu0FVECTOR pos) {
    ...
    KL2_OK_PRINT((1, 2, 18, scr_p[1], "minf = %f", minf));
    if (minf == 100000000.0f) {
        return -1;
    }
...
void DrawDebugRoute() {
    ...
    for (LP1 = 0; pRoute[LP1] != NULL; LP1++) {
        for (rtp = pRoute[LP1]; rtp->co != 0x7FFF && rtp->co != 0x7FFE; rtp++) {
            ...
            pp->size = 0;
            pp->buf = SPR_MEM_IDX(0);
            if (cnt & 1) {
                pp->buf = SPR_MEM_IDX(0x200);
            }
            pp->buf[pp->size].ul128 = 0;
            pp->buf[pp->size++].ul32[0] = DMAend | 5;
            pp->buf[pp->size].ul64[0] = SCE_GIF_SET_TAG(2, SCE_GS_TRUE, SCE_GS_TRUE, SCE_GS_SET_PRIM(SCE_GS_PRIM_LINE, 1, 0, 0, 0, 0, 0, 1, 0), SCE_GS_FALSE, 2);
            ...
            DmaChGIF->chcr.TTE = 1;
            sceDmaSend(DmaChGIF, SPR_SRC(pp->buf));
            cnt++;
        }
    }
}
