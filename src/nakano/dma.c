...
void nk_PrintOT(qword *tag) {
    ...
    do {
        ...
        qwc = (*tag)[0] & 0xFFFF;
        KL2_DEBUG_PRINT(("addr %08x tag %08x %08x %08x %08x %s qwc %d\n", (u32)tag, (*tag)[0], (*tag)[1], (*tag)[2], (*tag)[3], nk_dma_mode_name[mode >> 28], qwc));
        ...
        switch (mode) {
            ...
            case 0x50000000: // CALL
                if (ret_tag[1] != NULL) {
                    KL2_DEBUG_PRINT(("error call stack over flow\n"));
                    mode = 0x70000000;
                } else {
                    if (ret_tag[0] != NULL) {
                        ret_tag[1] = tag + 1;
