...
#define METER_WORK_SZ KL2_VER_COND(10, 20)
...
static nkMETER nkMeterWork[METER_WORK_SZ];
...
static nkMETER* nkMeter(s32 r, s32 g, s32 b) {
    ...
    if (nkDG.meter_num >= METER_WORK_SZ) {
        return NULL;
    }
...
void nkDrawWorkMeter() {
    ...
    pack->buf[pack->size++].ul32[0] = DMAend | nkDG.meter_num * 4 + 1;
    pack->buf[pack->size].ul64[0] = SCE_GIF_SET_TAG(nkDG.meter_num * 2, SCE_GS_TRUE, SCE_GS_TRUE, SCE_GS_SET_PRIM(SCE_GS_PRIM_SPRITE, 0, 0, 0, 0, 0, 0, 0, 0), SCE_GIF_PACKED, 2);
    ...
    DmaChGIF->chcr.TTE = 1;
    sceDmaSend(DmaChGIF, SPR_SRC(pack->buf));
