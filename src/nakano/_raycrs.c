...
s32 GetHitBlocks(sceVu0FVECTOR vf0, sceVu0FVECTOR vf1) {
    ...
    stb[0] = vmin[0] / KL2_VER_COND(hmi->gdw, grid[0]);
    stb[1] = vmin[1] / KL2_VER_COND(hmi->gdh, grid[1]);
    stb[2] = vmin[2] / KL2_VER_COND(hmi->gdw, grid[0]);
    ...
    edb[0] = vmax[0] / KL2_VER_COND(hmi->gdw, grid[0]);
    edb[1] = vmax[1] / KL2_VER_COND(hmi->gdh, grid[1]);
    edb[2] = vmax[2] / KL2_VER_COND(hmi->gdw, grid[0]);
    ...
    if (edb[0] >= vf[0])
        edb[0] = vf[0] - KL2_VER_COND(hmi->gdw, grid[0]);
    if (edb[1] >= vf[1])
        edb[1] = vf[1] - KL2_VER_COND(hmi->gdh, grid[1]);
    if (edb[2] >= vf[2])
        edb[2] = vf[2] - KL2_VER_COND(hmi->gdw, grid[0]);
    ...
    for (lpz = stb[2]; lpz <= edb[2]; lpz += KL2_VER_COND(hmi->gdw, grid[0])) {
        for (lpy = stb[1]; lpy <= edb[1]; lpy += KL2_VER_COND(hmi->gdh, grid[1])) {
            for (lpx = stb[0]; lpx <= edb[0]; lpx += KL2_VER_COND(hmi->gdw, grid[0])) {
                ret = GdCheck(ray[0], fw0[0], lpx, lpx + KL2_VER_COND(hmi->gdw, grid[0])) & GdCheck(ray[1], fw0[1], lpy, lpy + KL2_VER_COND(hmi->gdh, grid[1]));
                ret &= GdCheck(ray[2], fw0[2], lpz, lpz + KL2_VER_COND(hmi->gdw, grid[0]));
                if (ret != 0) {
                    hpadr = hr_hpmk(
                        lpx + KL2_VER_COND(hmi->gdw, grid[0]) / 2.0f + hmi->stx,
                        lpy + KL2_VER_COND(hmi->gdh, grid[1]) / 2.0f + hmi->sty,
                        lpz + KL2_VER_COND(hmi->gdw, grid[0]) / 2.0f + hmi->stz,
                        hmi->prim_top
                    );
                    if (hpadr != NULL) {
...
void nkMonKloHit(sceVu0FVECTOR posi) {
    ...
    KL2_OK_PRINT((1, 0, 22, scr_p[1], "KLO posi X:%f Y:%f Z:%f", posi[0] - hmi->stx, posi[1] - hmi->sty, posi[2] - hmi->stz));
    KL2_OK_PRINT((1, 0, 23, scr_p[1], "KLO block X:%f Y:%f Z:%f", (posi[0] - hmi->stx) / hmi->gdw, (posi[1] - hmi->sty) / hmi->gdh, (posi[2] - hmi->stz) / hmi->gdw));
}
