...
void NkDeb2PTbl() {
    kPadDATA *kpd1 = &GameGbl.kpd[1];

    if (GameGbl.flag & 4 || KL2_VER_COND(0, GameGbl.vision >> 8 >= 30)) {
        return;
    }
    ...
    if (nkDG.cam_debug_mode == 1) {
        KL2_OK_PRINT((1, 2, 7,  scr_p[1], "Follow Edit Camera"));
        KL2_OK_PRINT((1, 2, 8,  scr_p[1], "Game Camera"));
        KL2_OK_PRINT((1, 2, 9,  scr_p[1], "Fix Edit Camera"));
        KL2_OK_PRINT((1, 2, 10, scr_p[1], "----"));
        KL2_OK_PRINT((1, 2, 11, scr_p[1], "Light Edit <- coming soon!!"));
