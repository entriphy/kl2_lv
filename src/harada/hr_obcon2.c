...
// "initializer element is not a compile-time constant" who cares bro lmaoooooo
static PT96A hr_psw0100s1 = { 775, 1, 0, 1, 0x138A000 };
s32 hr_psw_0100[] = { 1, 
    (s32)&hr_psw0100s1, 2, 1
}; 

static PT96A hr_psw0104s0 = { 775, 1, 2, 1, 0x900000 };
static PT64A hr_psw0104s1 = { 776, 32, 0, 0 };
s32 hr_psw_0104[] = { 2,
    (s32)&hr_psw0104s0, 0, 1,
    (s32)&hr_psw0104s1, 2, 1
};

static PT96A hr_psw0400s1 = { 775, 0, 3, -1, 0x530000 };
s32 hr_psw_0400[] = { 1,
    (s32)&hr_psw0400s1, 2, 0
};

static PT96A hr_psw040bs0 = { 775, 0, 5, 1, 0x200000 };
static PT64A hr_psw040bs1 = { 776, 32, 0, 0 };
static PT64A hr_psw040bs2 = { 776, 33, 0, 0 };
s32 hr_psw_040b[] = { 3,
    (s32)&hr_psw040bs0, 0, 0,
    (s32)&hr_psw040bs1, 2, 2,
    (s32)&hr_psw040bs2, 4, 0
};

static PT64A hr_psw0505s0 = { 776, 32, 0, 0 };
static PT64A hr_psw0505s1 = { 776, 33, 0, 0 };
s32 hr_psw_0505[] = { 2,
    (s32)&hr_psw0505s0, 0, 4,
    (s32)&hr_psw0505s1, 2, 0
};

static PT96A hr_psw1004s0 = { 775, 0, 0, 1, 0x4E00000 };
s32 hr_psw_1004[] = { 1,
    (s32)&hr_psw1004s0, 0, 0
};
...
static s32 hr_start_gpt(s32 scene, s16 id) {
    ...
    ppmes.font = KL2_VER_COND(GetFHMAddress(hrpt_pack, scene + 1), hrpt_gms);
    hrptm_addr = (s32 *)-1;
    GameGbl.cam.mode = 7;
    if (id == 2) {
        hr_pt_set(2, (s16)scene / 2, 0, 0);
        KL2_DEBUG_PRINT(("game puppet %d %x %x %x\n", scene / 2, (u32)hrpt_addr, (u32)ppmes.font, (u32)ppwaku.gms));
        OkBalloonDie();
    } else {
        KL2_DEBUG_PRINT(("Alone puppet %d %x %x %x\n", scene / 2, (u32)hrpt_addr, (u32)ppmes.font, (u32)ppwaku.gms));
    }
    hr_pall_init();
    return 1;
}
...
static s32 hr_stsw_checkN KL2_VER_COND((PT64A *p64a), (PT64A *p64a, s32 stat1)) {
    s32 fg;

#ifdef KL2_VER_TRIAL
    fg = OkGetSwitchFlag(4, p64a->ss0) == 1;
#else
    ...
    KL2_OK_PRINT((4, 4, 20, scr_p[4], "sw %d %d", p64a->ss0, fg));
    if (p64a->ss1 == 1) {
        fg ^= 1;
    }
...
static void hr_gipt_check(OBJWORK *objw) {
    ...
    for (i = 0, flag = 1; i < objw->reg0; i++, flag <<= 1, tbl += 3) {
        ...
        // switch (*cmdno) {
        //     case 0x307:
        //         if (!hr_strt_check((PT96A *)cmdno, no, mc, herow->yuka_hit)) {
        //             continue;
        //         }
        //         break;
        //     case 0x308:
        //         if (!hr_stsw_checkN((PT64A *)cmdno, objw->stat1)) {
        //             continue;
        //         }
        //         break;
        //     default:
    ...
    if (objw->stat1 == 2) {
        KL2_OK_PRINT((4, 5, 15, scr_p[4], "gpReg1 %x", objw->reg1));
    } else {
        KL2_OK_PRINT((4, 5, 16, scr_p[4], "miniReg1 %x", objw->reg1));
    }
}

static void hr_gipt_draw(OBJWORK *objw) {
#ifdef KL2_VER_RETAIL
    s32 var = hr_check_skw();
    if (var) {
        hr_gipt_work(objw);
...
#ifdef KL2_VER_TRIAL
void hr_ptback_klo(s32 no, s32 mcn, s32 muki) {
    // Empty function
}
#else
...
static void (*hrGameInPtTbl[])(OBJWORK *) = {
    hr_gipt_check,
    hr_gipt_work,
    hr_gipt_tognd,
#ifdef KL2_VER_RETAIL
