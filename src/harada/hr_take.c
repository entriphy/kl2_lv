...
void hr_take_init() {
    ...
    if ((s32)addr != -1) {
        ...
        KL2_DEBUG_PRINT(("No Memory!!!!!(take)\n"));
    }
}
...
HOT* hr_take_workGet() {
    HOT *ho = getmem(sizeof(HOT));
    if ((s32)ho == -1) {
        ho = NULL;
        KL2_DEBUG_PRINT(("No Memory!!!!!!(take:getmem)\n"));
    } else {
...
void hr_take_modeldrawMir KL2_VER_COND((HOT *hot), (HOT *hot, s32 id)) {
    SFXOBJ *sfx;
#ifdef KL2_VER_TRIAL
    {
        void *ret = getmem(sizeof(SFXOBJ)); // Not in symbols
        sfx = ret;
        if ((s32)ret == -1) {
            return;
        }
        memcpy(ret, hot->pSfx, sizeof(SFXOBJ));
    }
#else
    ...
    ModelDraw(sfx);
    KL2_VER_COND(freemem, EraseSfxObject)(sfx);
}
