...
// 0: 60 < id < 64 || id == 4 || id == 5 || id >= 68
// 1: id < 4 || 5 < id < 60 || id < 68 || id > 69
...
void hr_call_efcdel(HR_CALL *ca, HR_PSYS *ps) {
    ...
    if (KL2_VER_COND(pte != PTE_HARADA, pte == PTE_ABE)) {
        KL2_DEBUG_PRINT(("pt efc del = abe\n"));
        ...
        KL2_DEBUG_PRINT(("pt efc del = harada\n"));
        if (ps->efc != NULL) {
...
static void hr_del_econt(HR_PSYS *ps, HR_ECONT *ec) {
    s32 pte = hr_efc_why(ec->id);
    if (KL2_VER_COND(pte != PTE_HARADA, pte == PTE_ABE)) {
        KL2_DEBUG_PRINT(("pt econt del = abe\n"));
        hr_abe_efcDel(ec->id);
    }
#ifdef KL2_VER_RETAIL
    else if (pte == PTE_HATO) {
        ...
        KL2_DEBUG_PRINT(("pt econt del = harada\n"));
        if (ps->efc != NULL) {
...
void hr_call_efcon(HR_CALL *ca, HR_PSYS *ps, s32 id, s32 cmd) {
    ...
    if (KL2_VER_COND(pte != PTE_HARADA, pte == PTE_ABE)) {
        ...
        KL2_DEBUG_PRINT(("pt efcON = harada\n"));
        if (ps->efc == NULL) {
...
void hr_callbuff_init(HR_PSYS *ps, s32 fg) {
    ...
    ps->ncall = 0;
    if ((s32)ps->call == -1) {
        KL2_DEBUG_PRINT(("NoMemory (pappet)!!!!!!\n"));
    } else {
        if (fg) {
            for (i = 0, ca = ps->call; i < 16; i++, ca++) {
                if (ca->flag != 0) {
...
void hr_psys_init(HR_PSYS *ps, s32 *addr, PCAMS *cam, s32 *addrm, HRPMOJI *pmes, HRPMWAKU *waku) {
    ...
    ps->idvc = -1;
    hr_ptbgm_init(ps);
    KL2_VER_TRIAL_ONLY(hrpt_vcntid = 0);
    KL2_DEBUG_PRINT(("puppet %x %x\n", (u32)addr, (u32)sizeof(HR_CALL)));
    KL2_VER_RETAIL_ONLY(addr++); // IMPORTANT: force jal rather than j
}
...
void pt_set_mesp(HR_CALL *ca, HR_PSYS *ps, u16 *addr, s16 time) {
    ...
    KL2_DEBUG_PRINT(("MESP ON t %d addr %x alpha %x\n", mesp->time, (u32)mesp->ptr, mesp->alpha));
...
s32 hr_psys_exit(HR_PSYS *ps) {
    ...
    if (hrpt_flag != 4) {
        ...
        if (ps->flag & 0x40) {
            ret = 3;
            KL2_VER_RETAIL_ONLY(if (hrpt_id != PTTITLE) ret |= 0x100);
        } else if (ps->flag & 0x10) {
            ret = 2;
            if (hrpt_view == 0 && KL2_VER_COND(hrpt_th == 0, 1)) {
                hr_ptback_klo KL2_VER_COND((GetMichiNo(ps->sys.rtw.rtp), ps->sys.rtw.mcn >> 12, ps->klov), (ps->klono, ps->klomc >> 12, ps->klov, ps->kloy, ps->flag));
            }
        } else {
            ret = 1;
        }
        ...
        if (ps->flag & 0x20) {
            ret |= 0x100;
            KL2_VER_RETAIL_ONLY(if (hrpt_id == PTTITLE) kzSetTPCFlag());
        }
#ifdef KL2_VER_RETAIL
    } else {
        ret = 4;
    ...
    KL2_DEBUG_PRINT(("exit No. %d, Michi %x\n", GetMichiNo(ps->sys.rtw.rtp), ps->sys.rtw.mcn));
    ...
    hr_pt_fclear();
    pt_free_mesp(ps);
    KL2_VER_RETAIL_ONLY(hr_pmes_exit(ps->pmes));
    KL2_VER_RETAIL_ONLY(hr_abe_efcClean());
    if (ret == 2) {
        hr_ptbgm_end(ps,0);
    }
...
void hr_pt_getrot(HR_CALL *ca, sceVu0FVECTOR *v) {
    sceVu0ScaleVectorXYZ(*v, (ca->rot).p, KL2_PI);
...
static void hr_call_efcwork(HR_CALL *ca, HR_PSYS *ps) {
    ...
    if (KL2_VER_COND(pte != PTE_HARADA, pte == PTE_ABE)) {
        ...
        sceVu0ScaleVector(hatrot, ca->rot.p, KL2_PI);
        sceVu0DivVector(hatrot, hatrot, 180.0f);
        hatrot[3] = 1.0f;
...
static void hr_work_nlight(sceVu0FVECTOR n, PCAMSC *pcc) {
    ...
    sceVu0ScaleVectorXYZ(v, pcc->p, KL2_PI);
    sceVu0DivVectorXYZ(v, v, 180.0f);
    v[3] = 1.0f;
...
void hr_ptmir_drawMain(HR_PSYS *ps) {
    ...
    for (i = 0, ca = ps->call; i < 16; i++, ca++) {
        ...
        } else if (ca->flag & 0x800) {
            if (ca->hObj != NULL) {
                hr_take_modeldrawMir KL2_VER_COND((ca->hObj), (ca->hObj, ca->chr));
            }
        } else {
            continue;
        }
...
void hr_psys_work(HR_PSYS *ps) {
    ...
    KL2_VER_RETAIL_ONLY(hr_loopse_work(ps));
    ca = &ps->sys;
    ...
    KL2_VER_TRIAL_ONLY(ps->count0++);
    KL2_VER_TRIAL_ONLY(ps->count1++);
    if (ps->flag & 6) {
        hr_pcam_motion(ps);
    }
    if (ps->flag & 8) {
...
void hr_ptlips_stop(HR_PSYS *ps) {
    KL2_DEBUG_PRINT(("LIPS OFF "));
    if (ps->flag & 0x400) {
        s16 *ptr = ps->addrlip;
        HR_CALL *cap = hr_id_search(ps, *(ptr - 2));
        KL2_DEBUG_PRINT(("%d %x", *(ptr - 2), (u32)cap));
...
void pt_set_skip(HR_PSYS *ps) {
    KL2_VER_RETAIL_ONLY(GameGbl.pause_flag2 = 1);
    KL2_VER_RETAIL_ONLY(hrpt_patch |= 4);
    KL2_VER_RETAIL_ONLY(hrpt_sbgm = 1);
    ps->flag |= 0x20;
    ps->skipcnt = 80;
#ifdef KL2_VER_TRIAL
    {
...
#endif
    KL2_VER_RETAIL_ONLY(hrpt_patch |= 2);
    hPptReset();
    hr_ptbgm_end(ps, 0);
    KL2_VER_RETAIL_ONLY(hr_hse_areaend());
}
...
s32 pt_flagj_P(u32 id) {
    ...
    return KL2_VER_COND(hr_pflag_check(mita), hr_pflag_checkSV(mita) ? 1 : 0);
}
...
void hr_ptvoice_call(HR_PSYS *ps, HRPMOJI *pmes) {
    ...
    if (ps->cntvc != 0) {
        if (pmes->flag & 0x400) {
            ...
            KL2_DEBUG_PRINT(("mesSpd %d %d Frm %dinter\n", pmes->time, pmes->hasv, frm));
        }

        ps->idvc = hPptReq();
        KL2_DEBUG_PRINT(("=== deb : ReqID %d <%d> ===\n", ps->idvc, SysGbl.irqc));
        KL2_VER_TRIAL_ONLY(hrpt_vcntid++);
        ps->addrvc++;
        ps->cntvc--;
    } else {
        KL2_DEBUG_PRINT(("!!!!!! Voice Over !!!!!!\n"));
    }

    KL2_VER_RETAIL_ONLY(ps->flag &= ~0x400);
    if (ps->cntlip != 0) {
        ptr = (s16 *)ps->addrlip;
        if (ptr[1] != 0) {
...
void hr_ptvoice_stop(HR_PSYS *ps, HRPMOJI *pmes) {
    s32 vid;
    if (ps->idvc != -1) {
        vid = KL2_VER_COND(hrpt_vcntid - 1 & 3, ps->idvc);
        hPptStop(vid);
        KL2_DEBUG_PRINT(("=== deb : StopID %d <%d> ===\n", ps->idvc, SysGbl.irqc));
    }
    hr_ptlips_stop(ps);
}
