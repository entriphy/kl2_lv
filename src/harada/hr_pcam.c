...
void hr_pcam_ang_init() {
    ...
    sceVu0DivVectorXYZ(hrpcam->wa.p, hrpcam->wa.p, KL2_PI);
}
...
void hr_pcam_mhold(PCAMSC *pcc, PCAMSM *pcm, sceVu0FMATRIX *m, s32 fg) {
    ...
    pcm->flag |= 0xB;
    pcm->flag &= KL2_VER_COND(~0x30, ~0x70);
}
...
void hr_pcam_vhold(PCAMSC *pcc, PCAMSM *pcm, sceVu0FVECTOR *v, s32 fg) {
    ...
    pcm->flag &= ~2;
    pcm->flag &= KL2_VER_COND(~0x30, ~0x70);
    if (fg == 2 || KL2_VER_COND(0, fg == 3)) {
        pcm->flag |= 0x30;
        KL2_DEBUG_PRINT(("vhold %f %f %f\n", pcm->hv[0], pcm->hv[1], pcm->hv[2]));
...
static void hr_pcam_posinte(PCAMSC *pcc, PCAMSM *pcm, sceVu0FVECTOR v) {
    ...
    } else if (pcm->flag & KL2_VER_COND(0x20, 0x60)) {
        if (pcm->flag & 0xC) {
            sceVu0CopyVectorXYZ(pcm->hv, *pcm->hdv);
        }
...
void hr_pcam_gyakusan(sceVu0FVECTOR *pi, PCAMSC *pcc, PCAMSM *pcm) {
    ...
    if (pcm->flag & 2) {
        sceVu0CopyVectorXYZ(v, pcm->hm[3]);
    } else {
        sceVu0CopyVector(v, pcm->hv);
    }
...
void hr_pcam_calc() {
    switch (hrpcam->flag) {
        case PCAMW_PIA:
            KL2_VER_RETAIL_ONLY(hr_pcam_ang(&hrpcam->wa, hrpcam->ang));
        ...
        default:
            ...
            hr_pcam_pi2rot(hrpcam->pos, hrpcam->inte, hrpcam->ang);
            hr_pcam_anglen();
            KL2_OK_PRINT((4, 2, 2, scr_p[4], "CAM DATA"));
            break;
    }
    hr_pcam_len(&hrpcam->proj);
    KL2_OK_PRINT((4, 2, 3, scr_p[4], "P %f %f %f", hrpcam->pos[0], hrpcam->pos[1], hrpcam->pos[2]));
    KL2_OK_PRINT((4, 2, 4, scr_p[4], "I %f %f %f", hrpcam->inte[0], hrpcam->inte[1], hrpcam->inte[2]));
    KL2_OK_PRINT((4, 2, 5, scr_p[4], "A %f %f %f", hrpcam->ang[0], hrpcam->ang[1], hrpcam->ang[2]));
}
...
void hr_pcam_work() {
    ...
    hr_pcam_calc();
    cam = &GameGbl.cam;
    if (KL2_VER_COND(1, hrpt_flag != 4 || ppsys.flag & 0x2000)) {
