...
void hr_mir_set() {
    ...
    hfmcam.posmode = 0;
    KL2_VER_RETAIL_ONLY(hfmcam.mpos[0] = hfmcam.mpos[1] = hfmcam.mpos[2] = 0.0f);
    KL2_VER_RETAIL_ONLY(hfmcam.mpos[3] = 1.0f);
    KL2_VER_RETAIL_ONLY(sceVu0CopyVector(hfmcam.mang, hfmcam.mpos));
    if (hfm_addr != NULL) {
        hr_mir_setf();
    }
    if (hcm_addr != NULL) {
    ...
    if ((s32)hrm1100 != -1) {
        ...
        if (KL2_VER_COND(1, hrm1100->flag & 2)) {
            hr_set_vpmfog(&hrm1100->infoB);
        }
    }

#ifdef KL2_VER_RETAIL
    if (hfmircnt != 0 && GameGbl.vision != KL2_VISION(11, 17)) {
        hfmcam.flag |= 2;
...
f32 hr_rot_pai(f32 r) {
    ...
    if (r > KL2_PI) {
        while (1) {
            ...
            r -= KL2_2PI;
            if (i >= 5) {
                KL2_OK_PRINT((4, 30, 32, scr_p[4], "RotPai %f %f", r0, r));
                break;
            }
        }
    } else if (r < -KL2_PI) {
        while (1) {
            ...
            r += KL2_2PI;
            if (i >= 5) {
                KL2_OK_PRINT((4, 34, 32, scr_p[4], "RotPai %f %f", r0, r));
                break;
            }
        }
...
static s32 hr_fmir_rtp(HFMIR *fmir, sceVu0IVECTOR *vert, u32 *sz) {
    ...
    for (i = 0, v = fmir->vert; i < fmir->count; i++, v++) {
        sceVu0RotTransPers(*vert, KL2_VER_COND(GameGbl.wsm, GameGbl.d_wsm), *v, 1);
        if (hr_mir_rtpc(vert)) {
            return 1;
        }
        vert++;
    ...
    sceVu0RotTransPers(iv, KL2_VER_COND(GameGbl.wsm, GameGbl.d_wsm), *fmir->center, 1);
    fmir->cx = iv[0] / 16.0f;
    fmir->cy = iv[1] / 16.0f;
    *sz = iv[2];
    return 0;
...
f32* hr_fmirver1_scale(HFMIR *fmir) {
    u32 size = ((u32)fmir - (u32)hfmirbuf) / sizeof(HFMIR);
    return hfm_scale + size * 2;
}

f32* hr_cmirver1_scale(HCMIR *cmir) {
    u32 size = ((u32)cmir - (u32)hcmirbuf) / sizeof(HCMIR);
...
static void hr_mir_cameraC(f32 *d, CAM_WORK *cam,  sceVu0FVECTOR *center, f64 mrx, f64 mry) {
    ...
    if (f_mrx == 0.0f) {
        ry = (f_mry - hr_atan2f(d[0], d[2])) * 2 + KL2_PI;
        sceVu0UnitMatrix(m);
    ...
    KL2_OK_PRINT((4, 3, 31, scr_p[4], "p %f %f %f", hfmcam.mpos[0], hfmcam.mpos[1], hfmcam.mpos[2]));
    KL2_OK_PRINT((4, 3, 32, scr_p[4], "c %f %f %f", (*center)[0], (*center)[1], (*center)[2]));
    KL2_OK_PRINT((4, 3, 33, scr_p[4], "r %f %f", f_mrx, f_mry));

    sceVu0UnitMatrix(m);
    sceVu0UnitMatrix(hfmcam.wvm);
    if (f_mrx == 0.0f) {
        ry = f_mry * 2 + KL2_PI + cam->ang[1];
        rx = hr_rot_pai(cam->ang[0]);
        ry = hr_rot_pai(ry);
        KL2_VER_RETAIL_ONLY(hfmcam.mang[0] = rx);
        KL2_VER_RETAIL_ONLY(hfmcam.mang[1] = -ry);
        sceVu0RotMatrixY(m, m, -ry);
        sceVu0RotMatrixX(m, m, rx);
    } else {
        sceVu0RotMatrixX(m, m, -f_mrx * 2);
        sceVu0RotMatrixY(m, m, (KL2_PI - f_mry) * 2);
        sceVu0RotMatrixY(m, m, hr_rot_pai(KL2_PI - cam->ang[1]));
        sceVu0RotMatrixX(m, m, cam->ang[0]);
        KL2_VER_RETAIL_ONLY(hfmcam.mang[0] = hr_rot_pai(cam->ang[0] - (f_mrx * 2)));
        KL2_VER_RETAIL_ONLY(hfmcam.mang[1] = hr_rot_pai(((KL2_PI - f_mry) * 2 + KL2_PI) - cam->ang[1]));
    }

    if (cam->ang[2] != 0.0f) {
        sceVu0RotMatrixZ(m, m, -cam->ang[2]);
        KL2_VER_RETAIL_ONLY(hfmcam.mang[2] = -cam->ang[2]);
    } else {
        KL2_VER_RETAIL_ONLY(hfmcam.mang[2] = 0.0f);
    }
    ...
    sceVu0MulMatrix(hfmcam.vsm, KL2_VER_COND(GameGbl.vsm, GameGbl.d_vsm), m);
...
static void hr_fmir_workD(sceVu0IVECTOR *vert, CAM_WORK *cam) {
    ...
    for (i = 0, v = wkfm->vert; i < wkfm->count; i++, v++) {
        sceVu0RotTransPers(vert[i], KL2_VER_COND(GameGbl.wsm, GameGbl.d_wsm), *v, 1);
    }
...
static s32 hr_cmir_rtp(HCMIR *cmir, u32 *sz) {
    ...
    for (i = 0, v = cmir->vert; i < cmir->count; i++, v++) {
        sceVu0RotTransPers(iv, KL2_VER_COND(GameGbl.wsm, GameGbl.d_wsm), *v, 1);
        if (hr_mir_rtpc(&iv)) {
            return 1;
        }
    }

    sceVu0RotTransPers(iv, KL2_VER_COND(GameGbl.wsm, GameGbl.d_wsm), cmir->center[cmir->men], 1);
    cmir->cx = iv[0] / 16.0f;
    cmir->cy = iv[1] / 16.0f;
    *sz = iv[2];
    return 0;
...
static void hr_cmir_hokan(HCMIR *cmir, CAM_WORK *cam, f32 maxinn) {
    ...
    sceVu0RotTransPers(iv, KL2_VER_COND(GameGbl.wsm, GameGbl.d_wsm), hfmcam.cent, 1);
    cmir->cx = iv[0] / 16.0f;
    cmir->cy = iv[1] / 16.0f;
}
...
static void hr_cmir_workD(CAM_WORK *cam) {
    ...
    hr_make_dum(&hfmcam.cent, wkcm->dst, wkcm->men, SPR_MEM, wkcm->count, 1);
    hr_mir_clipVPM(SPR_MEM, &hfmcam.cent, wkcm->count);

    for (iv = SPR_MEM, v = wkcm->vert, i = 0; i < wkcm->count; i++, v++, iv++) {
        sceVu0RotTransPers(*iv, KL2_VER_COND(GameGbl.wsm, GameGbl.d_wsm), *v, 1);
    }

    hr_mir_screenclipC(SPR_MEM, wkcm->count, wkcm->cx, wkcm->cy);
}
...
s32 func_0010DB60(s32 vision) {
    switch (vision) {
        case KL2_VISION(11, 4):
            return 3;
        case KL2_VISION(11, 1):
        case KL2_VISION(11, 2):
        case KL2_VISION(11, 6):
            return 1;
        default:
            return 0;
    }
...
vpmINFO* func_0010DBC0() {
    if ((s32)hrm1100 != -1) {
        ...
        if (GameGbl.vision == KL2_VISION(11, 4)) {
            if (hfmcam.type == 'F') {
                return &hrm1100->infoA;
            } else {
