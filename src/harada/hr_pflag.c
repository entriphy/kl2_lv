...
void hr_pflag_theatar(s32 id, s32 mode) {
    ...
    if (mode != 0) {
        switch (id) {
            case PT0706_S0:
                hr_pflag_on(PT0609_S0);
                break;
            case PT0609_S0:
                hr_pflag_on(PT0706_S0);
                break;
            default:
                return;
        }
...
s32 hr_pflag_check(s32 id) {
    ...
    } else if ((size = ptflag_buff[i] & id)) {
        return size;
    } else if ((size = ptflag_st[i] & id)) {
        return size;
    } else {
        return ptflag_ar[i] & id;
    }
}

void hr_pflag_on(s32 id) {
    s32 size = 32;
    s32 i = id / size;
...
s32 hr_pflag_get_id() {
    ...
    for (i = 0; i < ARR_COUNT_SIGNED(scene_table); i++) {
...
s16 hr_pflag_get_vs(s32 id) {
    ...
    p = scene_table;
    if (id >= ARR_COUNT_SIGNED(scene_table)) {
        hrpt_scene = 0;
        return 0;
    }
    p += id;
...
void hr_pflag_onS() {
    s32 id = KL2_VER_COND(hr_pflag_get_id(), hrpt_id);
    if (id != -1) {
        KL2_VER_COND(hr_pflag_on(id), hr_pflag_on(id, 1));
    }
}
...
#endif
        case 0x2801:
            if (KL2_VER_COND(hr_pflag_check, hr_pflag_checkSV)(PT4001_S0) == 0) {
                hr_pt_set(1, 0, mode, 0);
            } else if (KL2_VER_COND(hr_pflag_check, hr_pflag_checkSV)(PT4001_S1) == 0) {
                hr_pt_set(1, 1, mode, 0);
            } else if (KL2_VER_COND(hr_pflag_check, hr_pflag_checkSV)(PT4001_S2) == 0) {
                hr_pt_set(1, 2, mode, 0);
            } else if (KL2_VER_COND(hr_pflag_check, hr_pflag_checkSV)(PT4001_S3) == 0) {
                hr_pt_set(1, 3, mode, 0);
            }
#ifdef KL2_VER_RETAIL
