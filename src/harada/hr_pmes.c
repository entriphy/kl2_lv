...
void hr_pmes_init(HRPMOJI *pmes) {
    ...
    KL2_VER_RETAIL_ONLY(pmes->mema = getmem(0x300));
...
static void hr_pmes_waku_draw(HRPMWAKU *waku) {
    ATR_MWAKU *pp = SPR_MEM;
    s32 flag;

    KL2_VER_RETAIL_ONLY(flag = (s32)waku->gms != -1);
    if (KL2_VER_COND((s32)waku->gms != -1, !(waku->flag & 1) && flag)) {
        KL2_VER_TRIAL_ONLY(flag = 1);
        nkDmaSendVif1((qword *)waku->gms);
        KL2_VER_RETAIL_ONLY(waku->flag |= 1);
    } else {
        KL2_VER_TRIAL_ONLY(flag = 0);
    }

    pp->dmatag[0] = DMAend | (sizeof(ATR_MWAKU) / sizeof(qword) - 1);
...
static qword* hr_pmes_moji_drawM(HRPMOJI *pmes, ATR_MMOJIH *pp, ATR_MMOJI *pm, s32 x0, s32 y0, s32 alpha, s32 dx, s32 dy) {
    ...
    for (i = 0, count = 0; i < pmes->dcnt; i++) {
        if (!(*ptr & 0x8000)) {
            KL2_VER_RETAIL_ONLY(a = hr_pmes_mojiAlpha(pmes, alpha, count));
            KL2_VER_RETAIL_ONLY(dx = dx0);
            fw = hr_pmes_wsize(*ptr, &dx);
            hr_pmes_sprite(pm, x, y, 20, 96, *ptr, fw, KL2_VER_COND(alpha, a), dx, dy);
            x += fw + dx + 1;
            if (*ptr == 0x44) {
                x += 4;
            }
            ...
            switch (*ptr) {
                ...
                case (u16)MES_ITEM:
                    ptr++;
                    KL2_VER_RETAIL_ONLY(a = hr_pmes_mojiAlpha(pmes, alpha, count));
                    hr_pmes_sprite(pm, x, y, 80, 0, *ptr, 80, KL2_VER_COND(alpha, a), 0, 0);
                    x += 85;
                    count++;
                    pm++;
                    ptr++;
...
static void hr_pmes_moji_h KL2_VER_COND((ATR_MMOJIH *pp), (ATR_MMOJIH *pp, s32 id)) {
    pp->dmatag[0] = DMAend | (sizeof(ATR_MMOJIH) / sizeof(qword) - 1);
    pp->dmatag[1] = pp->dmatag[2] = pp->dmatag[3] = 0;
    ((u64 *)pp->gif0)[0] = SCE_GIF_SET_TAG(3, 0, 0, 0, 0, 1);
    pp->gif0[2] = SCE_GIF_PACKED_AD;
    pp->gif0[3] = 0;
    if (KL2_VER_COND(1, id == 0)) {
...
static void hr_pmes_moji_draw(HRPMOJI *pmes, s32 id) {
    ...
    flag = 0;
    if (KL2_VER_COND(1, !(pmes->flag & 0x1000)) && (s32)pmes->font != -1) {
        flag = 1;
        nkDmaSendVif1((qword *)pmes->font);
        KL2_VER_RETAIL_ONLY(pmes->flag |= 0x1000);
    }
    pp = SPR_MEM_IDX(13);
    KL2_VER_COND(hr_pmes_moji_h(pp), hr_pmes_moji_h(pp, 0));
    pk = (ATR_MKAS *)hr_pmes_moji_drawM(pmes, pp, (ATR_MMOJI *)(pp + 1), 40, KL2_VER_COND(168, 167), 128, 0, 0);
    if (id == 2) {
        pp->dmatag[0] += 9;
        pp->gif1[0] ^= 0x8000;
...
void hr_mesp_draw(HRMESP *mesp, HRPMOJI *pmes) {
    ...
    KL2_VER_COND(hr_pmes_moji_h((ATR_MMOJIH *)0x70000000), hr_pmes_moji_h((ATR_MMOJIH *)0x70000000, 1));
