...
static u8 hr_backcol(s32 time) {
    if (time < 0) {
        time = 0;
    } else if (time > KL2_VER_COND(128, 58)) {
        time = KL2_VER_COND(128, 58);
    }
    time = time * 255 / KL2_VER_COND(128, 58);
    return time;
}
...
void hr_ptefc_work(OBJWORK *objw) {
    ...
    for (i = 0, pt = objw->work; i < 4; i++, pt++) {
        if (pt->id != PTEFC_NON && pt->work != NULL) {
            pt->work(pt);
        }
    }

    KL2_OK_PRINT((4, 3, 16, scr_p[4], "pt efc"));
}
...
static HRPT_EFC* hr_ptefc_searchID(OBJWORK *objw, s32 id) {
    ...
    for (efc = objw->work, i = 0; i < 4; i++, efc++) {
        if (efc->id == id) {
            return efc;
        }
    }
...
static void hr_ptefc_copy(HRPT_EFC *efc, sceVu0FVECTOR pos, sceVu0FVECTOR rot) {
    ...
    sceVu0CopyVectorXYZ(efc->rot, rot);
    sceVu0ScaleVector(efc->rot, efc->rot, KL2_PI);
...
s32 hr_ptes_ON(s32 *ptr, s32 id, sceVu0FVECTOR pos, sceVu0FVECTOR rot) {
    HRPT_EFC *efc = hr_ptefc_searchID((OBJWORK *)ptr, PTEFC_NON);
    if (efc == NULL) {
        return 0;
    }
