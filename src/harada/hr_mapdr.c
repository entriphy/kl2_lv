...
void hrDrawMap() {
    ...
    KL2_OK_PRINT((4, 32, 1, scr_p[4], "block %d(%d) / %d", VpmInfo.block_list[0],hrmaxlcnt, VpmInfo.vpm_block_num));
    hrSetClipRoadParams();
...
VPOINFO* hrget_vpoinfo(s32 id) {
    if ((s32)hr_obcvpf == -1 || (s32)hr_obcvpo == -1) {
        return NULL;
    }

    if (hr_obcvpf == NULL || hr_obcvpo == NULL) {
        return NULL;
    }
...
static void hr_set_gsclear(ATR_HRCLR *pp) {
    pp->vif[0] = pp->vif[1] = 0;
    pp->vif[2] = (u32)VU0_MICRO;
    pp->vif[3] = SCE_VIF1_SET_DIRECT(KL2_VER_COND(7, 2), 0);
    ((u64 *)pp->gif)[0] = SCE_GIF_SET_TAG(KL2_VER_COND(6, 1), 1, 0, 0, 0, 1);
    pp->gif[2] = SCE_GIF_PACKED_AD;
    pp->gif[3] = 0;
...
static void hrInitAtrVu1mIvMir0() {
    ...
    pp->dmatag[0] = DMAcnt | (sizeof(ATR_VU1BACK) / sizeof(qword));
    pp->dmatag[1] = pp->dmatag[2] = pp->dmatag[3] = 0;
...
static void hrInitAtrVu1mIvMir1() {
    ...
    pp->dmatag[0] = DMAcnt | (sizeof(ATR_MIRR_1) / sizeof(qword) - 1);
    pp->dmatag[1] = pp->dmatag[2] = pp->dmatag[3] = 0;
...
static void hrBackAtrVu1mIvMir() {
    ...
    pp->dmatag[0] = DMAcnt | (sizeof(ATR_MIRR_RET) / sizeof(qword) - 1);
    pp->dmatag[1] = pp->dmatag[2] = pp->dmatag[3] = 0;
    env1 = (GameGbl.fr & 1) ? &GameGbl.db.draw01 : &GameGbl.db.draw11;
...
static void hr_mirpack_sub3(ATR_MIRR_FAN3 *pp, u32 count, u32 prim) {
    ...
    ((u64 *)pp->clamp1)[0] = SCE_GS_SET_CLAMP_2(5, 0, 0, 0, 0, 0);
    ((u64 *)pp->clamp1)[1] = SCE_GS_CLAMP_2;
...
#endif
        id = draw_id == 0;
        var_a0 = p1_ot_buff[id];
        nkDmaSendVif1(var_a0);
    }
}
#endif
