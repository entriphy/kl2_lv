...
void hr_crossline(sceVu0FVECTOR v, u32 wh, u32 r, u32 g, u32 b, u32 fg) {
    ...
    pp->dmatag[0] = DMAcnt | (sizeof(ATR_HRCL) / sizeof(qword) - 1);
    pp->dmatag[1] = pp->dmatag[2] = pp->dmatag[3] = 0;
    pp->viftag[0] = pp->viftag[1] = pp->viftag[2] = 0;
    pp->viftag[3] = SCE_VIF1_SET_DIRECT(13, 0);
    ((u64 *)pp->gif0)[0] = SCE_GIF_SET_TAG(4, SCE_GS_TRUE, SCE_GS_TRUE, SCE_GS_SET_PRIM(SCE_GS_PRIM_SPRITE, 0, 0, 0, 0, 0, 0, 0, 0), SCE_GIF_PACKED, 3);
...
void hrMapFileRead() {
    ...
    strcat(GmsName, ".gms");
    hr_gmsdata = getBuff(0, 0, GmsName, &i);
    if ((s32)hr_gmsdata != -1) {
...
void hrGetMapBuf() {
    ...
    VpmInfo.data_buff = getBuff(1, 0x200000, "map", &i);
}
...
static s32 MapvInit() {
    ...
    {
        s32 i;
        hrmvb = getBuff(1, sizeof(HR_MAPVS), "deb", &i);
    }
    hrGetMapBuf();
    hr_nak_init_allobj();
    nkVT_InitWave();
    ...
    hrmvb->mdmptr->tag2 = SCE_GS_RGBAQ | SCE_GS_XYZF2 << 4;
    ...
    hrmvb->clt_fg = 0;
#ifdef KL2_VER_TRIAL
    hrmvb->maps[0] = hrmvb->maps[1] = hrmvb->maps[2] = hrmvb->maps[3] = 0;
#else
...
void hrCamMonitor(CAM_WORK *cam) {
    ...
    ang[0] = (cam->ang[0] * 360.0f) / KL2_2PI;
    ang[1] = (cam->ang[1] * 360.0f) / KL2_2PI;
    ang[2] = (cam->ang[2] * 360.0f) / KL2_2PI;
    KL2_OK_PRINT((4, 1, 4, scr_p[4], "Cam Posi = %f : %f : %f", cam->posi[0], cam->posi[1], cam->posi[2]));
    KL2_OK_PRINT((4, 1, 5, scr_p[4], "Cam Ang  = %f : %f : %f", ang[0], ang[1], ang[2]));
}
...
static void hr_mv_onvram(u64 tex0, u32 x0, u32 y0, u32 x1, u32 y1, f64 s0, f64 t0, f64 s1, f64 t1) {
    ...
    pp = (HRVV *)p1_packet;
    pp->dmatag[0] = DMAcnt | (sizeof(HRVV) / sizeof(qword) - 1);
    pp->dmatag[1] = pp->dmatag[2] = pp->dmatag[3] = 0;
    pp->viftag[0] = pp->viftag[1] = pp->viftag[2] = 0;
    pp->viftag[3] = SCE_VIF1_SET_DIRECT(9, 0);

    ((u64 *)pp->gif0)[0] = SCE_GIF_SET_TAG(1, SCE_GS_FALSE, SCE_GS_FALSE, 0, SCE_GIF_PACKED, 1);
    pp->gif0[2] = SCE_GIF_PACKED_AD;
    pp->gif0[3] = 0;
    ((u64 *)pp->tex0)[0] = tex0;
    ((u64 *)pp->tex0)[1] = SCE_GS_TEX0_1;

    ((u64 *)pp->gif1)[0] = SCE_GIF_SET_TAG(2, SCE_GS_TRUE, SCE_GS_TRUE, SCE_GS_SET_PRIM(SCE_GS_PRIM_SPRITE, 0, 1, 0, 0, 0, 0, 0, 0), SCE_GIF_PACKED, 3);
    ((u64 *)pp->gif1)[1] = SCE_GS_ST | SCE_GS_RGBAQ << 4 | SCE_GS_XYZF2 << 8;
    rgb = 0x80;
    f = f_s0;
    pp->st0[0] = *(s32 *)&f;
    f = f_t0;
...
static void hr_mv_m_slint(s32 irep, f32 frep) {
    ...
    KL2_OK_PRINT((4, 44, 9,  scr_p[4], "X     %f", hrvlight[0].x));
    KL2_OK_PRINT((4, 44, 10, scr_p[4], "Y     %f", hrvlight[0].y));
    KL2_OK_PRINT((4, 44, 11, scr_p[4], "Z     %f", hrvlight[0].z));
    KL2_OK_PRINT((4, 44, 12, scr_p[4], "max R %f", hrvlight[0].r));
    KL2_OK_PRINT((4, 44, 13, scr_p[4], "    G %f", hrvlight[0].g));
    KL2_OK_PRINT((4, 44, 14, scr_p[4], "    B %f", hrvlight[0].b));
    KL2_OK_PRINT((4, 44, 15, scr_p[4], "near  %f", hrvlight[0].n));
    KL2_OK_PRINT((4, 44, 16, scr_p[4], "far   %f", hrvlight[0].f));
    KL2_OK_PRINT((4, 44, 17, scr_p[4], "min R %f", hrvlight[0].ri));
    KL2_OK_PRINT((4, 44, 18, scr_p[4], "    G %f", hrvlight[0].gi));
    KL2_OK_PRINT((4, 44, 19, scr_p[4], "    B %f", hrvlight[0].bi));
}
...
static void hr_mv_m_slint2(s32 irep, f32 frep) {
    ...
    KL2_OK_PRINT((4, 44, 18, scr_p[4], "    G %f", hrvlight[0].gi));
    KL2_OK_PRINT((4, 44, 19, scr_p[4], "    B %f", hrvlight[0].bi));
    KL2_OK_PRINT((4, 44, 20, scr_p[4], "X     %f", hrvlight[1].x));
    KL2_OK_PRINT((4, 44, 21, scr_p[4], "Y     %f", hrvlight[1].y));
    KL2_OK_PRINT((4, 44, 22, scr_p[4], "Z     %f", hrvlight[1].z));
    KL2_OK_PRINT((4, 44, 23, scr_p[4], "max R %f", hrvlight[1].r));
    KL2_OK_PRINT((4, 44, 24, scr_p[4], "    G %f", hrvlight[1].g));
    KL2_OK_PRINT((4, 44, 25, scr_p[4], "    B %f", hrvlight[1].b));
    KL2_OK_PRINT((4, 44, 26, scr_p[4], "near  %f", hrvlight[1].n));
    KL2_OK_PRINT((4, 44, 27, scr_p[4], "far   %f", hrvlight[1].f));
    KL2_OK_PRINT((4, 44, 28, scr_p[4], "min R %f", hrvlight[1].ri));
    KL2_OK_PRINT((4, 44, 29, scr_p[4], "    G %f", hrvlight[1].gi));
    KL2_OK_PRINT((4, 44, 30, scr_p[4], "    B %f", hrvlight[1].bi));
}

static HRANMVS hr_avt_tbl[] = { { NULL, hr_avw_sint, 3, &VpmInfo }, { NULL, hr_avw_sint2, 4, &VpmInfo } };
...
static void hr_mv_avt_menu(kPadDATA *kpd, s32 irep, f32 frep) {
    ...
    KL2_OK_PRINT((4, 42, hrmvb->avtmenu + 9, scr_p[4], ">"));
}
...
static void menu_clutwork(HR_CLUTS *ct, kPadDATA *kpd, s32 irep) {
    ...
    if (hrmvb->avtV != 0) {
        ...
        KL2_OK_PRINT((4, 33, ct->mvc_rgba + 9, scr_p[4], ">"));
        if (irep != 0) {
            hr_sendclut(gmsh);
        }
    }

    KL2_OK_PRINT((4, 28,                   8,                scr_p[4], "DBP %x X %d Y %d W %d H %d", gmsh->dbp, gmsh->dsax, gmsh->dsay, gmsh->rrw, gmsh->rrh));
    KL2_OK_PRINT((4, 34,                   9,                scr_p[4], "R:%02x", ((u8 *)rgba)[0]));
    KL2_OK_PRINT((4, 34,                   10,               scr_p[4], "G:%02x", ((u8 *)rgba)[1]));
    KL2_OK_PRINT((4, 34,                   11,               scr_p[4], "B:%02x", ((u8 *)rgba)[2]));
    KL2_OK_PRINT((4, 34,                   12,               scr_p[4], "A:%02x", ((u8 *)rgba)[3]));
    KL2_OK_PRINT((4, ct->mvc_rrw * 2 + 32, 13,               scr_p[4], "V"));
    KL2_OK_PRINT((4, 31,                   ct->mvc_rrh + 14, scr_p[4], ">"));
    hr_clutsp2(gmsh);
    KL2_OK_PRINT((4, 28,                   7,                scr_p[4], "%x X %d Y %d W %d H %d %d", gmsp->dbp, gmsp->dsax, gmsp->dsay, gmsp->rrw, gmsp->rrh, gmsp->dbw));
...
static void MapVConf(CAM_WORK *cam) {
    ...
    KL2_OK_PRINT((4, 2, 9,  scr_p[4], "Cam ScrZ = %f", cam->scr_z));
    KL2_OK_PRINT((4, 2, 10, scr_p[4], "BG UPPER COL  = R:%02x", mapvw.bgt.r));
    KL2_OK_PRINT((4, 2, 11, scr_p[4], "                G:%02x", mapvw.bgt.g));
    KL2_OK_PRINT((4, 2, 12, scr_p[4], "                B:%02x", mapvw.bgt.b));
    KL2_OK_PRINT((4, 2, 13, scr_p[4], "BG BOTTOM COL = R:%02x", mapvw.bgu.r));
    KL2_OK_PRINT((4, 2, 14, scr_p[4], "                G:%02x", mapvw.bgu.g));
    KL2_OK_PRINT((4, 2, 15, scr_p[4], "                B:%02x", mapvw.bgu.b));
    KL2_OK_PRINT((4, 2, 16, scr_p[4], "FOG COL = R:%02x", mapvw.fog_col[0]));
    KL2_OK_PRINT((4, 2, 17, scr_p[4], "          G:%02x", mapvw.fog_col[1]));
    KL2_OK_PRINT((4, 2, 18, scr_p[4], "          B:%02x", mapvw.fog_col[2]));
    KL2_OK_PRINT((4, 2, 19, scr_p[4], "FOG NEAR =%d", mapvw.fog_near));
    KL2_OK_PRINT((4, 2, 20, scr_p[4], "FOG FAR  =%d", mapvw.fog_far));
    KL2_OK_PRINT((4, 2, 21, scr_p[4], "SAVE"));
    KL2_OK_PRINT((4, 2, 22, scr_p[4], "OneBlock %d", hrmvb->ob_no));
    KL2_OK_PRINT((4, 2, 23, scr_p[4], "draw : %s", hrmvb->mdmbuf[hrmvb->mdm].name));
    KL2_OK_PRINT((4, 2, 24, scr_p[4], "LookType : %s", lookname[hrmvb->lookmode]));
    KL2_OK_PRINT((4, 2, 25, scr_p[4], "VPM : %s %d %d", mapbg_name[hrmvb->mvm_bg], hrcntbg, hrmvb->mvm_bgno));
    KL2_OK_PRINT((4, 2, 26, scr_p[4], "Mirror %f %f", hfmcam.scalex, hfmcam.scaley));
    KL2_OK_PRINT((4, 2, 27, scr_p[4], "Anm : %s", avt_name[hrmvb->avtcnt]));
    ct = &hrmvb->clsb;
    KL2_OK_PRINT((4, 2, 28, scr_p[4], "BG C %d/%d PIX %d/%d", ct->mv_clutn, ct->mv_clutc, ct->mv_pixn, ct->mv_pixc));
    ct = &hrmvb->cls;
    KL2_OK_PRINT((4, 2, 29, scr_p[4], "CLUT %d/%d PIX %d/%d", ct->mv_clutn, ct->mv_clutc, ct->mv_pixn, ct->mv_pixc));
    KL2_OK_PRINT((4, 2, 30, scr_p[4], "STS %d %d %d %d", hrmvb->maps[0], hrmvb->maps[1], hrmvb->maps[2], hrmvb->maps[3]));
    ...
    switch (MapVMenu) {
        ...
        case 14:
            ...
            if (hrmvb->mdm == 3) {
                hrmvb->fogcpos[0] += fh;
                hrmvb->fogcpos[2] += fv;
                KL2_OK_PRINT((4, 14, 23, scr_p[4], "%f %f %f", hrmvb->fogcpos[0], hrmvb->fogcpos[1], hrmvb->fogcpos[2]));
    ...
    KL2_OK_PRINT((4, 1, MapVMenu + 9, scr_p[4], ">"));
}
...
static void hrInitAtrVu1mIv_VO() {
    ...
    pp->dmatag[0] = DMAcnt | (sizeof(ATR_VU1M_IV) / sizeof(qword) - 1);
    pp->dmatag[1] = pp->dmatag[2] = pp->dmatag[3] = 0;
...
static s32 MapvMain() {
    ...
    KL2_OK_PRINT((4, 32, 3, scr_p[4], "SS %d %f", hfmcam.vsmFg, hfmcam.vsmSy));
    ...
    KL2_OK_PRINT((4, 1,  1, scr_p[4], "MapViewer"));
    KL2_OK_PRINT((4, 1,  2, scr_p[4], "IRQC = %d", SysGbl.irqc));
    KL2_OK_PRINT((4, 12, 1, scr_p[4], "< %s >", MapName));
    ...
    if (hrmvb->mvm_bg != 0) {
        KL2_OK_PRINT((4, 15, 6, scr_p[4], "bg"));
        for (i = 0, bg = hrbgbuff; i < hrcntbg; i++, bg++) {
            tfg[i] = bg->fg;
            if (bg->fg) {
                OkPrint(4, i * 10 + 18, 6, scr_p[4] ,"%d / %d,", bg->info->block_list[0], bg->info->vpm_block_num);
            }
    ...
    if (hrmvb->mvm_mtex != 0) {
        KL2_VER_RETAIL_ONLY(nkVT_ExecMovie());
        nkVT_Exec();
        hrSetMapREG();
        hrDrawMTex();
        nkP1Flush(&p1_ot[1]);
    ...
    nkSetMeter();
    nkDrawWorkMeter();
    OkPrint(4, 1, 3, scr_p[4], "Packet %x", (u32)p1_packet - psize);
...
static void init_hoshinoN() {
    hInitStage0();
    KL2_VER_RETAIL_ONLY(hSndSetMVol(1.0f));
}
...
s32 hrPtInit() {
    ...
    nkVT_InitWave();
    abEffectInit();
    KL2_VER_RETAIL_ONLY(ht_InitGbl());
    MapConfFileRead();
    KL2_VER_RETAIL_ONLY(hrPtSeLoad());
    hrDataLoad();
    hr_nak_dummyVK();
    hrInitWorkDeb();
    hr_anmVPM_allinit();
...
static void pri_mat2vec(PCAMSC *pcc, PCAMSM *pcm, s32 y) {
    ...
    KL2_OK_PRINT((4, 22, y + 0, scr_p[4], "%f", v[0]));
    KL2_OK_PRINT((4, 22, y + 1, scr_p[4], "%f", v[1]));
    KL2_OK_PRINT((4, 22, y + 2, scr_p[4], "%f", v[2]));
}
...
static void pri_pcam_pos(PCAMS *pcam, s32 y, s32 mv) {
    ...
    KL2_OK_PRINT((4, 3, y + 0, scr_p[4], "posiX %f", pcam->wp.p[0]));
    KL2_OK_PRINT((4, 3, y + 1, scr_p[4], "    Y %f", pcam->wp.p[1]));
    KL2_OK_PRINT((4, 3, y + 2, scr_p[4], "    Z %f", pcam->wp.p[2]));
    ret = pri_chk_vecmat(&pcam->mp, str);
    KL2_OK_PRINT((4, 3, y + 1, scr_p[4], "%s", str));
...
static void pri_pcam_inte(PCAMS *pcam, s32 y, s32 mv) {
    ...
    KL2_OK_PRINT((4, 3, y + 0, scr_p[4], "inteX %f", pcam->wi.p[0]));
    KL2_OK_PRINT((4, 3, y + 1, scr_p[4], "    Y %f", pcam->wi.p[1]));
    KL2_OK_PRINT((4, 3, y + 2, scr_p[4], "    Z %f", pcam->wi.p[2]));
...
static void pri_pcam_ang(PCAMS *pcam, s32 y) {
    KL2_OK_PRINT((4, 3, y + 0, scr_p[4], "ang X %f", pcam->wa.p[0]));
    KL2_OK_PRINT((4, 3, y + 1, scr_p[4], "    Y %f", pcam->wa.p[1]));
    KL2_OK_PRINT((4, 3, y + 2, scr_p[4], "len   %f", pcam->len.p));
}

static void pri_pcam_lr(PCAMSC *pcc, s32 y) {
    KL2_OK_PRINT((4, 3, y + 0, scr_p[4], "LR  X %f", pcc->p[0]));
    KL2_OK_PRINT((4, 3, y + 1, scr_p[4], "    Y %f", pcc->p[1]));
    KL2_OK_PRINT((4, 3, y + 2, scr_p[4], "    Z %f", pcc->p[2]));
}

static void pri_pcam_proj(PCAMS *pcam, s32 y) {
    KL2_OK_PRINT((4, 3, y + 0, scr_p[4], "proj   %f", pcam->proj.p));
}
