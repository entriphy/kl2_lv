...
s32 hr_pt_check() {
    return (hrpt_flag & KL2_VER_COND(0xB, 0x1B)) != 0;
}
...
static void hr_pt_LoadPack() {
    ...
    i = hrpt_scene * 2;
    if (i < *hrpt_pack) {
        hrpt_addr = GetFHMAddress(hrpt_pack, i);
        ppmes.font = KL2_VER_COND(GetFHMAddress(hrpt_pack, i + 1), hrpt_gms);
    }
}
...
void hr_pall_init() {
    ...
    KL2_VER_TRIAL_ONLY(hr_pflag_onS());
    ...
    hr_pmes_init(&ppmes);
    KL2_VER_RETAIL_ONLY(ppwaku.flag = 0);
    KL2_VER_RETAIL_ONLY(hrptm_res = 0);
    KL2_VER_RETAIL_ONLY(hrpt_patch = 0);
    if (hrpt_flag != 4) {
        hPptReset();
        KL2_VER_RETAIL_ONLY(hrpt_patch |= 1);
    }
    if (hrpt_flag != 4) {
        hrpt_stnk = 64;
    } else {
...
static void hr_pt_skipcheck(HR_PSYS *ps) {
    ...
    if (KL2_VER_COND(1, (0x3A < hrpt_id || (hrpt_id < 0x39 && hrpt_id != PTOPENING && hrpt_id != PT1304_S0) || (hrpt_view != 0 || sD->Stereo != SND_MODE_5P1CH))) && PAD_TRG_START(GameGbl.kpd[0])) {
        pt_set_skip(ps);
    }
}
...
void hr_pall_aftdraw() {
    sceGsSyncPath(0, 0);
    if (KL2_VER_COND(hrpt_flag != 8, !(hrpt_flag & 0x18))) {
