...
hEvOBJECT *pEvObj[64] __attribute__((section(".bss")));
hEvTABLE EvTbl[20][128] __attribute__((section(".bss")));
hEvRESTART EvRestart[20][128] __attribute__((section(".bss")));
...
void hEvObjMake(hEvTABLE *pt) {
    hEvOBJECT *pEO = pEvObj[ObjNum] = &EvObj[hWorkGet(1)];
...
void hEvObjMukiChange(hEvOBJECT *pEO) {
    ...
    for (i = 0; i < 2; i++) {
        rot[i] += (pTH->DefRot[i] + pTB->Rot[i]) * KL2_PI / 180.0f;
        if (pEO->Frame == 0) {
...
void hEvObjBankChange(hEvOBJECT *pEO) {
    hEvTHEAD *pTH = pEO->pTbl->pHd;
    hEvTBODY *pTB = &pEO->pTbl->pBd[pEO->TblIdx];
    pEO->pSfx->pMot->pBaseCoord->Rot[2] = (pTH->DefRot[2] + pTB->Bank[0]) * KL2_PI / 180.0f;
    pEO->dRot[2] = ((pTB->Bank[1] - pTB->Bank[0]) * KL2_PI / 180.0f) / pEO->Time;
...
void hEvObjMove(hEvOBJECT *pEO) {
    ...
    SetBaseMatrix(pEO->pSfx, pEO->pSfx->pMot->pBaseCoord->Rot, pEO->pSfx->pMot->pBaseCoord->Trans, KL2_VER_COND(0, 1));
}
        ...
        if (pEO->Wait < 1) {
            if (pEO->Frame2 == 0) {
                ...
                if (idx == 0) {
                    ...
                    hEvObjMoveInit(pEO);
                    pEO->MotLoop = 0;
                    KL2_VER_RETAIL_ONLY(pEO->seFlag = 0);
                    pEO->Mcn = 0;
                    pEO->Light = pTB->Light;
                    pEO->LightBak = -1;
                } else {
...
#ifdef KL2_VER_TRIAL
                    if (GetActCnt(pEO->pSfx) == 0.0f && pTB->SEprog != -1) {
                        hSeKeyOnEv(pTB->SEprog, pTB->SEsplt, pEO->pSfx->pMot->pBaseCoord->Trans, 0);
                    }
#else
...
void hEvObjDraw() {
    ...
    for (i = 0; i < ObjNum; i++) {
        ...
        if (GameGbl.pause_flag != 0) {
            ...
            if (pEO->LightBak != -1) {
                pEO->LightCnt++;
                if (pEO->LightCnt >= KL2_VER_COND(32.0f, 120.0f)) {
                    pEO->LightBak = -1;
                }
            }
...
void hEvTblInit(s32 addr, s32 ar) {
    ...
    pEv = GetFHMAddress(pEv, 1);
    tblMax[ar] = GetFHMNum(pEv);
    KL2_DEBUG_PRINT(("tblmax=%d\n", tblMax[ar]));
...
void hEvAreaInit() {
    ...
    KL2_DEBUG_PRINT(("hEvAreaInit ... %04x\n", GameGbl.vision));
    NoEvent = 0;
    
    if (NoStageData != 0) {
        KL2_DEBUG_PRINT(("NO STAGE EVENT DATA !\n"));
        NoEvent = 1;
        return;
    }
    ...
    KL2_DEBUG_PRINT(("hEvAreaInit ... done\n"));
}
