...
#ifdef KL2_VER_TRIAL
EFXSE efdm[] = {
    {0, 0.0f, 0, 0, 1, 0.0f, 0.0f, 0, 0}
};

EFXSE ef0100[] = {
    {4, 0.22f, 0, 0, 1, 0.0f, 0.0f, 0, 0}
};

EFXSE ef0101[] = {
    {5, 0.6f, 0, 0, 1, 0.0f, 0.0f, 0, 0}
};

EFXSE ef0102[] = {
    {4, 0.2f, 0, 0, 1, 0.0f, 0.0f, 0, 0}
};

EFXSE ef0103[] = {
    {4, 0.6f, 0, 0, 1, 0.0f, 0.0f, 0, 0}
};

EFXSE ef0104[] = {
    {4, 0.05f, 0, 0, 1, 0.0f, 0.0f, 0, 0}
};

EFXSE ef0500[] = {
    {4, 0.15f, 0, 0, 1, 0.0f, 0.0f, 0, 0}
};

EFXSE ef0501[] = {
    {3, 0.3f, 0, 0, 1, 0.0f, 0.0f, 0, 0},
};

EFXSE ef0504[] = {
    {4, 0.3f, 0, 0, 1, 0.0f, 0.0f, 0, 0},
};
...
EFXSE ef0505[] = {
    {4, 0.1f, 0, 0, 1, 0.0f, 0.0f, 0, 0}
};

EFXSE *EfxSE[50][20] = {
    { efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm },
    { ef0100, ef0101, ef0102, ef0103, ef0104, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm },
    { efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm },
    { efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm },
    { efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm, efdm },
...
u8 SndPacket[0x400] __attribute__((aligned(128), section(".bss")));
u8 SndTempBuff[0x100000] __attribute__((aligned(128), section(".bss")));
hSNDDATA SndData = {};
hSNDDATA *sD = NULL;
...
void hSndReset() {
    ...
    sD->fadeFlag = 0;
    sD->Mute = 0;
    KL2_DEBUG_PRINT(("hSndReset ... done\n"));
}

void hSndFadeOutAll(s32 frame) {
    KL2_VER_RETAIL_ONLY(hSeKeyOffAll());
    sD->fadeFlag = 1;
    sD->fadeCnt = 0.0f;
...
void hSndMain() {
    ...
    if (sD->TitleDelayCnt != 0 && --sD->TitleDelayCnt == 0) {
        hSeKeyOn(KL2_VAG(0, 105, 1, 0, 10, 0x0C, 200, 200), NULL, 0);
        hSeKeyOn(KL2_VAG(1, 105, 1, 0, 10, 0x0C, 200, 200), NULL, 0);
    }
#endif
    ...
    if (sD->effVol != sD->effVolBak && sD->effChange == 0) {
        if (sD->effVol - sD->effVolBak > KL2_VER_COND(0.1f, 0.01f)) {
            sD->effVolBak += KL2_VER_COND(0.1f, 0.01f);
        } else if (sD->effVol - sD->effVolBak < -KL2_VER_COND(0.1f, 0.01f)) {
            sD->effVolBak -= KL2_VER_COND(0.1f, 0.01f);
        } else {
...
void hSndInit() {
    KL2_VER_RETAIL_ONLY(sceSdRemoteInit());
    ...
    sD->effMix = 3;
    hSndPkEffect();
    sD->effVolBak = KL2_VER_COND(1.0f, 0.0f);
    sD->effVol = 0.0f;
    hSndPkSetEVol(0);
    sD->envNum = 0;
    KL2_VER_RETAIL_ONLY(hSeLock(0));
    hSeInitGrp(0);
    sD->seMVol = KL2_VER_COND(0.88188976f, 1.43f);
    sD->seObjID = 0;
    sD->bgmMVol = KL2_VER_COND(0.92913383f, 0.708661437f);
    sD->pptMVol = KL2_VER_COND(0.4566929f, 0.314960629f);
    KL2_VER_RETAIL_ONLY(sD->TitleDelayCnt = 0);
...
void hSndBankSet(s32 packaddr, s32 id) {
    ...
    s32 tblnum;
    s32 tblsize;
    KL2_VER_RETAIL_ONLY(char stack[16]);
    
    if (id != 0) {
        tbladdr = (s32)GetFHMAddress((u32 *)packaddr, 2);
        tblsize = *(s16 *)tbladdr * 2;
        tblnum = sD->stageTblNum = tblsize / 2;
        memcpy(sD->stageTbl, (void *)(tbladdr + 2), tblsize);
        KL2_DEBUG_PRINT(("tblnum=%d ", tblnum));
    }
    ...
    KL2_DEBUG_PRINT(("hdsize=%d bdsize=%d\n", hdsize, bdsize));
...
void hSndBankSetMain() {
    ...
    KL2_DEBUG_PRINT(("hSndBankSetMain ... done\n"));
}

void hSndBankSetStage() {
    s32 addr;

    KL2_DEBUG_PRINT(("hSndBankSetStage ...\n"));
    addr = hGetDataAddr(2);
    if (addr != 0) {
        sD->stageBank = 1;
        hSndBankSet(addr, 1);
        ...
        sD->stageBank = 0;
        KL2_DEBUG_PRINT(("NO STAGE BANK DATA\n"));
    }
    KL2_DEBUG_PRINT(("hSndBankSetStage ... done\n"));
}
...
void hSndEffSetArea() {
    ...
    KL2_VER_RETAIL_ONLY(sD->effIdx = 0);
    sD->effVol = eff->vol;
    sD->effVolBak = KL2_VER_COND(1.0f, 0.0f);
    sD->effMode = eff->efx;
    sD->effDepth = 0x7fff;
...
void hSndEffSetVol(KL2_VER_COND(s32, f32) vol) {
    sD->effVol = vol;
}
