...
static s32 sbuff[16] __attribute__((aligned(64)));
static s32 sndBuff[16] __attribute__((aligned(64)));
...
void hSndRpcRet() {
    ...
    str = &ret->STRinfo;
    KL2_VER_RETAIL_ONLY(bD->iopPoint = str->BGMpoint);
    ...
    KL2_VER_RETAIL_ONLY(aD->AC3stat = str->AC3stat);
}
...
s32 hRpcInit() {
    ...
    do {
        if (sceSifBindRpc(&gCd, 0x12346, 0) < 0) {
            KL2_DEBUG_PRINT(("error: sceSifBindRpc\n"));
            while (1);
        }
        for (i = 10000; i != -1; i--) {
            // Do nothing
