...
void ViewInit() {
    ...
    var_a1 = (u32)ALIGN(DataBuffPtr);
    SfxDataBuff[0] = (u8 *)var_a1;
    SfxDataBuff[1] = (u8 *)(var_a1 + 0x300000);
    SfxDataBuff[2] = (u8 *)(var_a1 + 0x600000);
    SfxDataBuff[3] = (u8 *)(var_a1 + 0x900000);
    DataBuffPtr = (u32 *)(var_a1 + 0xC00000);
    DataBuffPtr = ALIGN(DataBuffPtr);

#ifdef KL2_VER_TRIAL
    size = ReadFile("host0:../../dat/chr/spec1.gim", (qword *)DataBuffPtr);
    pSpecGim = (s16 *)DataBuffPtr;
    DataBuffPtr = ALIGN((u32)DataBuffPtr + size);
    SfxTexInfoIndex = 0;
    GimLoad((u32 *)pSpecGim, SfxTexInfo);
    SfxTexInfoIndex++;
#else
    GameGbl.vision = KL2_VISION(1, 0);
    abGimInit();
#endif
...
void View() {
    ...
    KL2_OK_PRINT((0, 2, 1,scr_p[0],"ModelViewer Ver 0.00001"));
    KL2_OK_PRINT((0, 2, 2, scr_p[0], "Mode = "));
    KL2_OK_PRINT((0, 9, 2, scr_p[0], "%s", ModeName[ViewMode]));
    ...
    for (i = 0; i < 4; i++) {
        if (vObj[i].pObj != NULL) {
            ...
            KL2_OK_PRINT((0, i * 11 + 2, 4, scr_p[0], "DATA%d %s", i, ReadFileName));
        } else {
            KL2_OK_PRINT((0, i * 11 + 2, 4, scr_p[0], "DATA%d NOP", i));
        }

        if (vObj[i].AllFlag != 0) {
            KL2_OK_PRINT((0, i * 11 + 7, 4, scr_p[0], "*", i));
        }
    }
    KL2_OK_PRINT((0, ModelIndex * 11 + 1, 4, scr_p[0], ">"));
    ...
    hrDrawVpo_end();
    sceGsSyncPath(0, 0);
    KL2_OK_PRINT((0, 3, 34, scr_p[0], "TotalCnt %d", *T0_COUNT));
}
...
static void ViewRead_() {
    ...
    for (i = 0; i < pBinh->DirNum; i++) {
        KL2_OK_PRINT((0, i / 24 * 8 + 2, i % 24 + 7, scr_p[0], "%s", pBinh->DirName[i]));
    }
    KL2_OK_PRINT((0, DirNum / 24 * 8 + 1, DirNum % 24 + 7, scr_p[0], ">"));
    
    for (i = 0; i < pBinh->FileNum[DirNum]; i++) {
        KL2_OK_PRINT((0, i / 24 * 4 + 27, i % 24 + 7, scr_p[0], "%s", pBinh->FileName[DirNum][i]));
    }
    KL2_OK_PRINT((0, ChrNum / 24 * 4 + 26, ChrNum % 24 + 7, scr_p[0], ">"));
...
static void ViewAction_() {
    ...
    if (vObj[ModelIndex].pObj != NULL) {
        ...
        if (GameGbl.kpd[0].trg & (PAD_L1 | PAD_L2)) {
    ...
    for (i = 0; i < 4; i++) {
        if (vObj[i].pObj != NULL) {
            ...
            KL2_OK_PRINT((0, i * 11 + 2, 5, scr_p[0], "CNT=%d", TmpCnt));
            ...
            if (pObjAct != NULL && pObjAct->pMot->CoordNum != 0) {
                ...
                KL2_OK_PRINT((0, i * 11 + 2, 6, scr_p[0], "%s", ActName));
                KL2_OK_PRINT((0, i * 11 + 2, 7, scr_p[0], "Number=%d", vObj[i].ActNum));
                TmpCnt = GetActEndCnt(pObj);
                KL2_OK_PRINT((0, i * 11 + 2, 8, scr_p[0], "%d", TmpCnt));
                TmpCnt = GetActCnt(pObj);
                KL2_OK_PRINT((0, i * 11 + 2, 9, scr_p[0], "%d", TmpCnt));
            }
        }
    }
    ...
    if (vObj[ModelIndex].pObj != NULL) {
     ...
     for (i = 0; i < 4; i++) {
        if (vObj[i].pObj != NULL) {
            ...
            KL2_OK_PRINT((0, i * 11 + 2, 10, scr_p[0], "Ypos=%d", (s32)(pObj->ClipOffset / -2.4)));
            KL2_OK_PRINT((0, i * 11 + 2, 11, scr_p[0], "Zone=%d", (s32)(pObj->ClipZone / 2.4)));
        }
    }
}
