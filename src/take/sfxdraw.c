        ...
        if (pObj->OutLineFlag != 0) {
            if (bboxTmp[1][2] > 1 && bboxTmp[1][2] < 0xFFFFF) {
                u128 *packet;
                {
                    ...
                    KL2_VER_RETAIL_ONLY(__asm__("sync"));
                    PacketKickPath3((u32)tmp_buf, ((u32)packet - (u32)tmp_buf) / 16);
                }

                sceVu0CopyVector((f32 *)OutlineEnv.bbox[0], (f32 *)bboxTmp[0]);
                sceVu0CopyVector((f32 *)OutlineEnv.bbox[1], (f32 *)bboxTmp[1]);
...
// Not matching on retail: https://decomp.me/scratch/RTkE0
static void SfxPacketMakeLight(SFXOBJ *pObj, s32 PartsNum, u64 *pStrip, u64 *pUv, u128 *pColorI, u128 *pVertexI, u128 *pSpecUVI, s32 LastStripNum) {
    s32 adc_cnt = 0;
    ...
    KL2_VER_COND(sceDmaSync(pDma.Gif, 0, 0), DmaSync_Mac(pDma.Gif));
    DmaSend_Mac(pDma.Gif, SPR_SRC(SPR_MEM_IDX(0x220)));
    ...
    KL2_VER_COND(DmaSync_Mac(pDma.Gif), sceDmaSync(pDma.Gif, 0, 0));
